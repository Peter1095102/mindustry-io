...
public class JoinDialog extends FloatingDialog{
    ...
    public JoinDialog(){
        ...
        add.buttons.addButton("$ok", () -> {
            if(renaming == null){
                ...
                /*Server defServer = new Server();  // figure out a way to add the default server without making it invasive
                defServer.setIP("mindustry.me:5000");
                servers.add(defServer);
                saveServers();
                setupRemote();
                refreshRemote();*/
            }else{
    ...
    private Array<Server> defaultServerArray = new Array<>();
    private void drawDefaultServer(){
        Server server1 = new Server();
        server1.setIP("mindustry.io:5000");
        defaultServerArray.add(server1);

        for(Server server : defaultServerArray){
            remote.clear();
